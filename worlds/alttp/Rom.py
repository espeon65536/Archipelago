...
def get_nonnative_item_sprite(item: str, game: str) -> int:
    if game == 'SMZ3':
        if 'Card' in item:
            if 'Boss' in item:
                return 0x32
        ...
        if 'SmMap' in item:
            return 0x33
        elif 'BigKey' in item or 'Key' in item or 'Map' in item or 'Compass' in item:
            # lookup ID from SMZ3's enum
            from worlds.smz3.TotalSMZ3.Item import ItemType as SMZ3ItemType
            return getattr(SMZ3ItemType, item)
    elif game == 'Ocarina of Time':
        if 'Boss Key' in item:
            return 0x32
        elif 'Small Key' in item:
            return 0xAF
        elif 'Map' in item:
            return 0x33
        elif 'Compass' in item:
            return 0x25
            ...
            if location.item is not None:
                if not location.native_item:
                    if location.item.trap:
                        ...
                        itemid = get_nonnative_item_sprite(location.item.name, location.item.game)
    ...
    for shop_id, shop in enumerate(shops):
        ...
        for index, item in enumerate(shop.inventory):
            ...
            if item['player'] and world.game[item['player']] != "A Link to the Past":  # item not native to ALTTP
                item_code = get_nonnative_item_sprite(item['item'], world.game[item['player']])
            else:
